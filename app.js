...
client.on("ready", () => {
    ...
    const statuses = [ // status bot
        "Music",
        ` Music with ${client.guilds.cache.size} servers`,
        ` Music with ${client.guilds.cache.reduce((a, b) => a + b.memberCount, 0)} users`,
        "Youtube",
        "/play",
        "Spotify",
        "Discord",
...
client.distube
    .on("playSong", (queue, song) => {
        const embed = new MessageEmbed()
            .setColor("RANDOM")
            .setAuthor({ name: "Starting to play the track!", iconURL: "https://raw.githubusercontent.com/HELLSNAKES/Music-Slash-Bot/main/assets/music.gif" })
            .setThumbnail(song.thumbnail)
            .setDescription(`[${song.name}](${song.url})`)
            .setFooter({ text: `Muxi loves UG! Playing the song on request of ${song.user.username}`, iconURL: song.user.avatarURL() })
            .setTimestamp()
        queue.textChannel.send({ embeds: [embed] })
    })
    .on("addSong", (queue, song) => {
        const embed = new MessageEmbed()
            .setTitle("Sucessfully added song to queue")
            .setDescription('Added successfully')
            .setColor("RANDOM")
            .setFooter({ text: `Muxi is cool!`})
            .setTimestamp()
        queue.textChannel.send({ embeds: [embed] })
    })
    .on("addList", (queue, playlist) => {
        const embed = new MessageEmbed()
            .setTitle("Successfully added playlist")
            .setDescription(`Added \`${playlist.name}\` playlist (${playlist.songs.length} songs) to queue\n${status(queue)}`)
            .setColor("RANDOM")
            .setFooter({ text: `Muxi is not dumb!`})
            .setTimestamp()
        queue.textChannel.send({ embeds: [embed] })
    })
    .on("error", (textChannel, e) => {
        console.error(e)
        textChannel.send(`An error has been encountered: ${e}`)
    })
    // .on("finish", queue => queue.textChannel.send("***No more song in queue. Leaving the channel***"))
    .on("finishSong", queue => {
        const embed = new MessageEmbed()
            .setDescription(`I finished playing \`${queue.songs[0].name}\``)
        queue.textChannel.send({ embeds: [embed] })
    })
    .on("disconnect", queue => {
        const embed = new MessageEmbed()
    ...
    .on("empty", queue => {
        const embed = new MessageEmbed()
            .setDescription("There is no one to listen music. Leaving channel to save resources.")
        queue.textChannel.send({ embeds: [embed] })
    })
    .on("initQueue", (queue) => {
